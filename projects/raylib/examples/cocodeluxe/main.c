// main.c — Squares + Circles + per-shape textures + twist-to-rotate + music loop
#include "raylib.h"
#include <math.h>
#include <stdlib.h>
...
static const float BALL_RADIUS_MAX = 100.0f;
static const float SPEED_MIN       = 1.0f;
static const float SPEED_MAX       = 30.0f;

static const float SQUARE_MIN_SIDE = 20.0f;
static const float SQUARE_MAX_SIDE = 600.0f;
...
// Texture file list (index with texId; -1 means no texture)
#define TEX_COUNT 5
...
// One row = one object.
// { type,  x,   y,   sizePx, angleDeg, texId, fit,           tint }
typedef struct {
    ShapeType type;
    float     x, y;
...
// *** Assign shapes + textures here ***
static const ShapeInit SHAPES_PRESET[NUM_SHAPES] = {
    { SHAPE_CIRCLE,  40, 120, 200,  0, 0, TEX_FIT_COVER,   WHITE },
    { SHAPE_CIRCLE, 240, 420, 200,  0, 1, TEX_FIT_COVER,   WHITE },
    { SHAPE_CIRCLE, 540, 320, 200,  0, 2, TEX_FIT_COVER,   WHITE },
...
// ----- Types -----
// Put Shape BEFORE any function that uses it (e.g., audio helpers)
...
// ---------- Gesture-safe audio + size→pitch (AFTER Shape is defined) ----------
...
static void StartLoopMusic(void){
    if (!gAudioReady) EnsureAudioReady();           // makes sure audio device is up
    if (!gMusicLoaded){
        gLoop = LoadMusicStream("assets/audio/loop1.mp3");
        if (gLoop.ctxData != NULL){
            SetMusicVolume(gLoop, 0.35f);           // tweak to taste
            PlayMusicStream(gLoop);
            gMusicLoaded  = 1;
            gMusicPlaying = 1;
        }
    } else if (!gMusicPlaying){
        PlayMusicStream(gLoop);
        gMusicPlaying = 1;
    }
}
...
static void DrawShapeWithTexture(const Shape *sh){
    if (sh->type == SHAPE_SQUARE){
        ...
        if (TextureIndexOk(sh->texId)){
            ...
            DrawTexturePro(tex, src, dest, org, sh->angle, sh->tint); // geometry & texture rotated
        } else {
        ...
        if (TextureIndexOk(sh->texId)){
            ...
            DrawTexturePro(tex, src, dest, org, ang, sh->tint); // rotate image on circle
        } else {
    ...
    while (!WindowShouldClose()){
        ...
        if (touchCount == 0){
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)){
                StartLoopMusic(); // first desktop interaction
                dragMouseShape = TopShapeAt(mpos.x, mpos.y, shapes, NUM_SHAPES);
                if (dragMouseShape != -1) PlayTapInForShape(&shapes[dragMouseShape]); else { EnsureAudioReady(); StartLoopMusic(); PlaySound(gTapIn); }
            }
            ...
            if (IsMouseButtonPressed(MOUSE_RIGHT_BUTTON)){
                ...
                if (rotateMouseShape != -1) PlayTapInForShape(&shapes[rotateMouseShape]); else { EnsureAudioReady(); StartLoopMusic(); PlaySound(gTapIn); }
            }
            ...
            if (effectiveCount == 1){
                ...
                if (prevTouchCount == 0){
                    ...
                    if (dragTouchShape != -1) PlayTapInForShape(&shapes[dragTouchShape]); else { EnsureAudioReady(); StartLoopMusic(); PlaySound(gTapIn); }
                }
                if (dragTouchShape != -1){
                ...
                if (prevTouchCount < 2 && pinchShape == -1){
                    ...
                    pinchShape = sIdx;
                    if (pinchShape != -1) PlayTapInForShape(&shapes[pinchShape]); else { EnsureAudioReady(); StartLoopMusic(); PlaySound(gTapIn); }
                    pinchActive = 0;
                }
        ...
        // ---------- Music stream pump ----------
        if (gMusicLoaded) UpdateMusicStream(gLoop);
    }
