...
int main(void){
    ...
    while (!WindowShouldClose()){
        ...
        if (touchCount == 0){
            ...
            // --- Mouse wheel: scale by default; rotate if R is held ---
            ...
            if (wheel != 0.0f){
                ...
                if (IsKeyDown(KEY_R)) {
                    // R-wheel rotate (desktop only). Works for squares (geom) and circles (texture if ROTATE_TEXTURES).
                    shapes[idx].angle += wheel * 4.0f;   // tweak sensitivity as desired
                } else {
                    // Default: scale
