...
// Function to add special colored sphere obstacles
function addSpecialObstacles(scene, amount, area) {
    ...
    const colorValues = [
        { color: 0xff69b4, healthValue: 5 }, // Pink
        { color: 0x1e90ff, healthValue: 3 }, // Blue
        { color: 0xffff00, healthValue: 2 }, // Yellow
        { color: 0xffffff, healthValue: 1 }, // White
        { color: 0xc0c0c0, healthValue: 4 }, // Silver
        { color: 0x000000, healthValue: -5 }, // Black (negative value)
    ];

    for (let i = 0; i < amount; i++) {
        const radius = 1.5 + Math.random() * 1.5; // Random size between 1.5 and 3
        const sphereGeometry = new THREE.SphereGeometry(radius, 32, 32);

        const randomColorIndex = Math.floor(Math.random() * colorValues.length);
        const selectedColor = colorValues[randomColorIndex];

        const sphereMaterial = new THREE.MeshStandardMaterial({
            color: selectedColor.color, // Set sphere color
            metalness: 0.8, // Adjust reflectivity
            roughness: 0.1, // Adjust surface roughness
            envMapIntensity: 1.0, // Reactive to environment maps
        });
        ...
        // Store health value and bounding box
        sphere.userData = {
            healthValue: selectedColor.healthValue,
    ...
    console.log(`${amount} special obstacles added within an area of ${area} units.`);
}
...
// Configuration object for tire tracks
const tireTrackConfig = {
    color: 0x333333,    // Track color
    initialOpacity: 0.15, // Initial opacity of the tire tracks
    fadeDuration: 1,    // Time in seconds for the track to fully fade out
    trackWidth: 0.25,    // Track width
    trackLength: 0.5    // Track length
};
...
const minSpeedForTracks = 0.5; // Minimum speed to emit tracks
let lastLeftTrackPosition = null;
let lastRightTrackPosition = null;
const trackSpacing = 0.1; // Minimum distance between tracks

function emitTireTracks() {

    if (!wheels.backLeft || !wheels.backRight) {
        //console.warn("Wheels are not fully loaded; skipping tire track emission.");
        return;
    }

    const speed = Math.abs(vehicleVelocity.length()); // Get current speed
    if (speed < minSpeedForTracks) return; // Skip if the car is barely moving

    const backLeftPosition = wheels.backLeft.getWorldPosition(new THREE.Vector3());
    const backRightPosition = wheels.backRight.getWorldPosition(new THREE.Vector3());

    // Ensure tracks "grow" smoothly
    if (lastLeftTrackPosition && backLeftPosition.distanceTo(lastLeftTrackPosition) < trackSpacing) return;
    if (lastRightTrackPosition && backRightPosition.distanceTo(lastRightTrackPosition) < trackSpacing) return;

    addTireTrack(backLeftPosition);
    addTireTrack(backRightPosition);

    lastLeftTrackPosition = backLeftPosition.clone();
    lastRightTrackPosition = backRightPosition.clone();
}

function addTireTrack(position) {
    const geometry = new THREE.PlaneGeometry(tireTrackConfig.trackWidth, tireTrackConfig.trackLength); // Track shape
    const material = new THREE.MeshBasicMaterial({
        color: tireTrackConfig.color,
        opacity: tireTrackConfig.initialOpacity,
        transparent: true,
        depthWrite: false,
    });
    const track = new THREE.Mesh(geometry, material);

    track.quaternion.copy(vehicleContainer.quaternion); // Align with the car's direction
    track.rotateX(-Math.PI / 2); // Flat on the ground
    track.position.set(position.x, 0.163, position.z); // Set position slightly above the ground

    scene.add(track);
    tireTrackSegments.push({ mesh: track, lifetime: tireTrackConfig.fadeDuration, initialOpacity: tireTrackConfig.initialOpacity });
...
function updateTireTracks(deltaTime) {
    for (let i = tireTrackSegments.length - 1; i >= 0; i--) {
        ...
        const fadeFactor = trackSegment.lifetime / tireTrackConfig.fadeDuration; // Calculate fade factor
        trackSegment.mesh.material.opacity = trackSegment.initialOpacity * fadeFactor; // Apply proportional fade-out
...
// let steeringSoundContext = null;
// let noiseBuffer = null; // White noise buffer
// let steeringNoiseSource = null;
// let steeringGainNode = null;
// let bandpassFilter = null;
// let distortionNode = null;
// let lfoGainNode = null; // Modulation for squeaking effect

// const steeringSoundConfig = {
//     baseFrequency: 400, // Center frequency for bandpass filter
//     maxFrequency: 14000, // Maximum frequency during sharp turns
//     minGain: 0.0, // Minimum volume
//     maxGain: 2, // Maximum volume during sharp turns
//     turnThreshold: Math.PI / 25, // Trigger sound when steering angle exceeds this threshold (~7 degrees)
//     fadeOutTime: 0.2, // Time to fade out when the turn ends
//     modulationRate: 300, // Modulation rate (Hz) for the squeak pulses
//     distortionAmount: 50, // Amount of distortion for a breaking effect
// };

// // **Initialize WebAudio context for steering sound**
// function initSteeringSound() {
//     if (!steeringSoundContext) {
//         steeringSoundContext = new (window.AudioContext || window.webkitAudioContext)();

//         // Create white noise buffer
//         noiseBuffer = createWhiteNoiseBuffer(steeringSoundContext);

//         // Create gain and filter nodes
//         steeringGainNode = steeringSoundContext.createGain();
//         steeringGainNode.gain.setValueAtTime(0, steeringSoundContext.currentTime); // Start muted

//         bandpassFilter = steeringSoundContext.createBiquadFilter();
//         bandpassFilter.type = "bandpass"; // Bandpass filter for focused squeaky sound
//         bandpassFilter.frequency.setValueAtTime(steeringSoundConfig.baseFrequency, steeringSoundContext.currentTime);

//         // Create distortion node for a rougher sound
//         distortionNode = steeringSoundContext.createWaveShaper();
//         distortionNode.curve = makeDistortionCurve(steeringSoundConfig.distortionAmount);
//         distortionNode.oversample = "4x";

//         // LFO for amplitude modulation (squeaking effect)
//         const lfo = steeringSoundContext.createOscillator();
//         lfo.type = "sine";
//         lfo.frequency.setValueAtTime(steeringSoundConfig.modulationRate, steeringSoundContext.currentTime); // Modulation rate in Hz

//         lfoGainNode = steeringSoundContext.createGain();
//         lfoGainNode.gain.setValueAtTime(0.7, steeringSoundContext.currentTime); // Modulation depth
//         lfo.connect(lfoGainNode);
//         lfo.start();

//         // Connect LFO to the gain node to create the "squeak" pulses
//         lfoGainNode.connect(steeringGainNode.gain);

//         // Connect nodes
//         steeringGainNode.connect(bandpassFilter);
//         bandpassFilter.connect(distortionNode);
//         distortionNode.connect(steeringSoundContext.destination);
//     }
// }
...
//     for (let i = 0; i < bufferSize; i++) {
//         data[i] = Math.random() * 2 - 1; // Generate white noise [-1, 1]
//     }
...
// // **Update function called in the main update loop**
// function updateSteeringSound(deltaTime, steeringAngle) {
//     if (!steeringSoundContext) return; // Ensure the audio context is initialized

//     const absSteeringAngle = Math.abs(steeringAngle);
//     if (absSteeringAngle > steeringSoundConfig.turnThreshold) {
//         if (!steeringNoiseSource) {
//             steeringNoiseSource = steeringSoundContext.createBufferSource();
//             steeringNoiseSource.buffer = noiseBuffer;
//             steeringNoiseSource.loop = true;
//             steeringNoiseSource.connect(steeringGainNode);
//             steeringNoiseSource.start();
//         }

//         // Calculate frequency and volume (gain)
//         const steeringRatio = Math.min(absSteeringAngle / Math.PI, 1); // Normalize to [0, 1]
//         const frequency = steeringSoundConfig.baseFrequency +
//             (steeringSoundConfig.maxFrequency - steeringSoundConfig.baseFrequency) * steeringRatio;
//         const gain = steeringSoundConfig.minGain + (steeringSoundConfig.maxGain - steeringSoundConfig.minGain) * steeringRatio;

//         // Apply to bandpass filter and gain
//         bandpassFilter.frequency.setValueAtTime(frequency, steeringSoundContext.currentTime);
//         steeringGainNode.gain.setTargetAtTime(gain, steeringSoundContext.currentTime, 0.1); // Smoothly increase gain
//     } else {
//         if (steeringNoiseSource) {
//             // Fade out sound and stop noise when the turn ends
//             steeringGainNode.gain.setTargetAtTime(0, steeringSoundContext.currentTime, steeringSoundConfig.fadeOutTime);
//             setTimeout(() => {
//                 if (steeringNoiseSource) {
//                     steeringNoiseSource.stop();
//                     steeringNoiseSource.disconnect();
//                     steeringNoiseSource = null;
//                 }
//             }, steeringSoundConfig.fadeOutTime * 1000);
//         }
//     }
// }

// // Stop the steering sound when it's no longer needed
// function stopSteeringSound() {
//     if (steeringNoiseSource) {
//         steeringNoiseSource.stop();
//         steeringNoiseSource.disconnect();
//         steeringNoiseSource = null;
//     }
//     if (steeringSoundContext) {
//         steeringSoundContext.close();
//         steeringSoundContext = null;
//     }
// }

// window.initSteeringSound = initSteeringSound;
// window.updateSteeringSound = updateSteeringSound;
// window.stopSteeringSound = stopSteeringSound;
...
let steeringNoiseSource = null;
let steeringGainNode = null;
let steeringLowPassFilter = null;

const steeringSoundConfig = {
    baseCutoffFrequency: 100, // Base frequency for the low-pass filter
    maxCutoffFrequency: 15000, // Higher pitch during sharp turns
    minGain: 0.0, // Minimum volume
    maxGain: 2, // Maximum volume during sharp turns
    turnThreshold: Math.PI / 25, // Trigger sound when steering angle exceeds this threshold (~7 degrees)
    fadeOutTime: 0.25, // Time to fade out when the turn ends
};
...
function initSteeringSound() {
    if (!steeringSoundContext) {
        ...
        steeringLowPassFilter = steeringSoundContext.createBiquadFilter();
        steeringLowPassFilter.type = "lowpass"; // Low-pass filter for muffled tire sound
        steeringLowPassFilter.frequency.setValueAtTime(steeringSoundConfig.baseCutoffFrequency, steeringSoundContext.currentTime);

        // Connect nodes
        steeringGainNode.connect(steeringLowPassFilter);
        steeringLowPassFilter.connect(steeringSoundContext.destination);
    }
}
...
function updateSteeringSound(deltaTime, steeringAngle) {
    ...
    if (absSteeringAngle > steeringSoundConfig.turnThreshold) {
        ...
        // Calculate pitch (cutoff frequency) and volume (gain)
        const steeringRatio = Math.min(absSteeringAngle / Math.PI, 1); // Normalize to [0, 1]
        const cutoffFreq = steeringSoundConfig.baseCutoffFrequency +
            (steeringSoundConfig.maxCutoffFrequency - steeringSoundConfig.baseCutoffFrequency) * steeringRatio;
        const gain = steeringSoundConfig.minGain + (steeringSoundConfig.maxGain - steeringSoundConfig.minGain) * steeringRatio;

        // Apply to filter and gain
        steeringLowPassFilter.frequency.setValueAtTime(cutoffFreq, steeringSoundContext.currentTime);
        steeringGainNode.gain.setTargetAtTime(gain, steeringSoundContext.currentTime, 0.1); // Smoothly increase gain
    } else {
        if (steeringNoiseSource) {
            // Fade out sound and stop noise when the turn ends
...
let brakingSoundContext = null;
let brakingNoiseBuffer = null;
let brakingNoiseSource = null;
let brakingGainNode = null;
let brakingLowPassFilter = null;
let surfaceType = "gravel"; // Default surface type (can be set dynamically)

const brakingSoundConfig = {
    baseCutoffFrequency: 40, // Base frequency for the low-pass filter
    maxCutoffFrequency: 5400, // Higher pitch for stronger braking
    minGain: 0.0, // Minimum volume
    maxGain: 0.025, // Maximum volume
    fadeOutTime: 0.4, // Time to fade out when braking ends
    speedThreshold: 2.0, // Minimum speed (m/s) for the braking sound to play
    surfaceModifiers: {
        asphalt: { gainMultiplier: 1.0, pitchOffset: 0 },
        gravel: { gainMultiplier: 1.5, pitchOffset: 200 },
        sand: { gainMultiplier: 0.8, pitchOffset: -100 },
        wet: { gainMultiplier: 1.2, pitchOffset: 100 },
    }
};

// **Initialize WebAudio context for braking sound**
function initBrakingSound() {
    if (!brakingSoundContext) {
        brakingSoundContext = new (window.AudioContext || window.webkitAudioContext)();

        // Create white noise buffer
        brakingNoiseBuffer = createWhiteNoiseBuffer(brakingSoundContext);

        // Create gain and filter nodes
        brakingGainNode = brakingSoundContext.createGain();
        brakingGainNode.gain.setValueAtTime(0, brakingSoundContext.currentTime); // Start muted

        brakingLowPassFilter = brakingSoundContext.createBiquadFilter();
        brakingLowPassFilter.type = "lowpass"; // High-pass filter for sharper brake sound
        brakingLowPassFilter.frequency.setValueAtTime(brakingSoundConfig.baseCutoffFrequency, brakingSoundContext.currentTime);

        // Connect nodes
        brakingGainNode.connect(brakingLowPassFilter);
        brakingLowPassFilter.connect(brakingSoundContext.destination);
    }
}

// **Start braking sound only when speed exceeds threshold**
function startBrakingSound(vehicleSpeed) {
    if (vehicleSpeed < brakingSoundConfig.speedThreshold) return; // Skip if speed is too low

    const intensity = vehicleSpeed / 30; // Normalize speed to 0-1 for intensity
    const modifiers = brakingSoundConfig.surfaceModifiers[surfaceType] || { gainMultiplier: 1.0, pitchOffset: 0 };

    if (!brakingNoiseSource) {
        brakingNoiseSource = brakingSoundContext.createBufferSource();
        brakingNoiseSource.buffer = brakingNoiseBuffer;
        brakingNoiseSource.loop = true;
        brakingNoiseSource.connect(brakingGainNode);
        brakingNoiseSource.start();
    }

    // Adjust pitch and gain based on surface and speed
    const cutoffFreq = brakingSoundConfig.baseCutoffFrequency + brakingSoundConfig.maxCutoffFrequency * intensity + modifiers.pitchOffset;
    const gain = brakingSoundConfig.maxGain * intensity * modifiers.gainMultiplier;

    brakingLowPassFilter.frequency.setValueAtTime(cutoffFreq, brakingSoundContext.currentTime);
    brakingGainNode.gain.setTargetAtTime(gain, brakingSoundContext.currentTime, 0.05); // Quick ramp-up for braking sound
}

// **Stop braking sound when brakes are released**
function stopBrakingSound() {
    if (brakingNoiseSource) {
        brakingGainNode.gain.setTargetAtTime(0, brakingSoundContext.currentTime, brakingSoundConfig.fadeOutTime);
        setTimeout(() => {
            if (brakingNoiseSource) {
                brakingNoiseSource.stop();
                brakingNoiseSource.disconnect();
                brakingNoiseSource = null;
            }
        }, brakingSoundConfig.fadeOutTime * 1000);
    }
}

// **Create white noise buffer (used for general friction noise)**
function createWhiteNoiseBuffer(context) {
    const bufferSize = context.sampleRate * 1; // 1 second of noise
    const buffer = context.createBuffer(1, bufferSize, context.sampleRate);
    const data = buffer.getChannelData(0);
...
// **Key press handling for braking sound**
document.addEventListener('keydown', (event) => {
    if (event.key === 'b') {
        const vehicleSpeed = Math.abs(vehicleVelocity.length()); // Get current speed (m/s)
        startBrakingSound(vehicleSpeed); // Play braking sound if speed is above the threshold
    }
});

document.addEventListener('keyup', (event) => {
    if (event.key === 'b') {
        stopBrakingSound(); // Stop braking sound when key is released
    }
});

// **Surface Type Change (optional)**
function setSurfaceType(type) {
    if (brakingSoundConfig.surfaceModifiers[type]) {
        surfaceType = type;
        console.log(`Surface type set to: ${type}`);
    } else {
        console.warn(`Surface type "${type}" is not recognized.`);
    }
}

window.initBrakingSound = initBrakingSound;
window.startBrakingSound = startBrakingSound;
window.stopBrakingSound = stopBrakingSound;
window.setSurfaceType = setSurfaceType;

// Example usage
// setSurfaceType("gravel"); // Changes to gravel sounds
...
let radioGainNode = null; // Gain node for volume control
let currentTrackSource = null; // Current playing track source

const radioTracks = [
    'audio/tracks/Jagugoo.mp3',
    'audio/tracks/Jagugoo1.mp3',
    'audio/tracks/Jagugoo2.mp3',
    'audio/tracks/Jagugoo3.mp3',
    'audio/tracks/Jagugoo4.mp3',
    'audio/tracks/NeedForSpeed.mp3'
];
...
function initRadio() {
    ...
    if (!radioGainNode) {
        radioGainNode = audioContext.createGain(); // Gain node for controlling volume
        radioGainNode.gain.setValueAtTime(0.5, audioContext.currentTime); // Default volume
...
function playNextTrack() {
    if (currentTrackSource) {
        currentTrackSource.mediaElement.pause(); // Use pause() instead of stop()
        currentTrackSource.disconnect(); // Disconnect to avoid multiple sources
        currentTrackSource = null;
    }

    // Select the next track (avoid repetitions)
    ...
    const trackUrl = radioTracks[currentTrackIndex];
    const audioElement = new Audio(trackUrl);
    audioElement.loop = false;

    currentTrackSource = audioContext.createMediaElementSource(audioElement);
    currentTrackSource.connect(radioGainNode); // Connect to gain node for volume control

    audioElement.play();
    audioElement.addEventListener('ended', playNextTrack); // Auto-play next track on end
}
...
function toggleRadio() {
    ...
    if (radioEnabled) {
        ...
        if (currentTrackSource) {
            currentTrackSource.mediaElement.pause();
...
function setRadioVolume(volume) {
    if (radioGainNode) {
        radioGainNode.gain.setValueAtTime(volume, audioContext.currentTime);
        console.log(`Radio volume set to: ${volume}`);
    }
}

// // Add event listeners for volume control
// document.getElementById('radioVolumeSlider').addEventListener('input', (event) => {
//     const volume = parseFloat(event.target.value);
//     setRadioVolume(volume); // Set volume based on slider input
// });

// Add key binding to toggle radio
document.addEventListener('keydown', (event) => {
    if (event.key.toLowerCase() === 't') {
        toggleRadio(); // Toggle radio on/off
    }
});
...
function playNegativeHealthSound() {
    if (!audioContext) initAudioContext(); // Ensure audio context is initialized

    const duration = 1; // Duration of the sound
    const oscillator = audioContext.createOscillator(); // Create an oscillator
    const gainNode = audioContext.createGain(); // Create a gain node to control volume

    // Configure oscillator for a "buzzy" warning sound
    oscillator.type = "square"; // Square wave for harsher sound
    oscillator.frequency.setValueAtTime(20, audioContext.currentTime); // Starting frequency (A3 note)

    // Create a frequency dip for a "descending" negative tone
    oscillator.frequency.linearRampToValueAtTime(10, audioContext.currentTime + duration); // End frequency (lower pitch)

    // Configure gain (quick fade out)
    gainNode.gain.setValueAtTime(0.4, audioContext.currentTime); // Initial gain (volume)
    gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + duration); // Fade-out
...
//window.playNegativeHealthSound = playNegativeHealthSound;
...
function checkCollisions() {
    ...
    if (!modelsLoaded) return; // Skip collision checks if models are not fully loaded
    ...
    specialObstacleGroup?.children.forEach((sphere) => {
        ...
        if (carBoundingBox.intersectsBox(sphereBoundingBox)) {
            const healthValue = sphere.userData.healthValue; // Get the health value associated with the sphere
            healthPoints += healthValue;  // Adjust health points based on sphere's health value

            console.log(
                `Collected a ${healthValue >= 0 ? "positive" : "negative"} health sphere. Health change: ${healthValue}, Total health: ${healthPoints}`
            );

            spheresToRemove.push(sphere);  // Mark for removal

            if (healthValue < 0) {
                playNegativeHealthSound();  // Play damage sound for negative health values
            } else {
                playHealthPickupSound();  // Play health pickup sound for positive health values
            }

            updateUI();  // Update the health UI with the latest health points
        }
    });
...
Object.entries(glbFilesArray).forEach(([modelName, modelUrl], index, array) => {
    gltfLoader.load(
        modelUrl,
        (gltf) => {
            ...
            if (index === array.length - 1) {
                modelsLoaded = true; // Mark as loaded after the last model
...
let maxSpeed = 250.0 / 3.6; // Convert to m/s
let deceleration = 8;
let brakeForce = 20.0;
...
let fuelCapacity = 30; // In liters for fuel car or kWh for electric (assume 60-liter fuel tank or 60 kWh battery)
let currentFuel = fuelCapacity; // Start with a full tank/battery
const fuelConsumptionRate = 14; // Liters per 100 km (can switch to kWh if electric)
let totalDistanceTraveled = 0; // Total distance in km
const kmMeterElement = document.getElementById("km-meter"); // Assuming you have an HTML element to show km
const fuelMeterElement = document.getElementById("fuel-meter"); // Assuming an HTML element for fuel display


function updateFuelConsumption(deltaTime) {
    if (currentFuel <= 0) {
        console.warn("Out of fuel! Vehicle cannot move.");
        return; // Stop the vehicle if out of fuel
    }

    const speedKmPerSecond = (vehicleVelocity.z * 3.6) / 1000; // Convert m/s to km/s
    const distanceThisFrame = speedKmPerSecond * deltaTime; // Distance traveled in this frame in km

    totalDistanceTraveled += distanceThisFrame; // Accumulate total distance
    const fuelConsumed = (distanceThisFrame * fuelConsumptionRate) / 100; // Calculate fuel consumed for this distance

    currentFuel -= fuelConsumed; // Reduce fuel
    if (currentFuel < 0) currentFuel = 0; // Prevent negative fuel

    // Update the UI (if applicable)
    if (kmMeterElement) kmMeterElement.innerText = `Distance: ${totalDistanceTraveled.toFixed(2)} km`;
    if (fuelMeterElement) fuelMeterElement.innerText = `Fuel: ${currentFuel.toFixed(2)} L`;
}
...
document.addEventListener('keydown', (event) => {
  ...
  if (event.key.toLowerCase() === 'a') {
    ...
    function updateScene() {
        ...
        if (!modelsLoaded) {
            //console.warn("Models not fully loaded; skipping scene update.");
            return;
        }
        ...
        updateSteeringSound(deltaTime, steeringAngle);
        //updateBrakingSound(deltaTime, deceleration);
