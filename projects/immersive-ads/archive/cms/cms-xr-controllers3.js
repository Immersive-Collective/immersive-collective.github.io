...
// Toggle camera follow mode on key press "f"
document.addEventListener('keydown', (event) => {
    if (event.key === 'f') {
        isCameraFollowingCar = !isCameraFollowingCar;
        controls.enabled = !isCameraFollowingCar; // enable/disable OrbitControls
...
function updateCameraFollow(deltaTime) {
    if (isCameraFollowingCar && !isCameraMoving && !isFlying) {
        ...
        // For a speed-based lerp factor, for instance:
        const carSpeed = Math.abs(vehicleVelocity.z);
        const lerpFactor = Math.min(0.02 + (carSpeed / maxSpeed) * 0.3, 1);

        // Target camera position behind/above the car
        const targetPosition = carWorldPos.clone().add(initialFollowOffset);
        camera.position.lerp(targetPosition, lerpFactor);

        // Make the camera look at the car
        const lookAtPos = carWorldPos.clone().add(cameraLookAtOffset);
        camera.lookAt(lookAtPos);

    } else {
        // === FOLLOW MODE OFF ===
        // Keep camera transform exactly as it was upon exit
        camera.position.copy(followModeLastCameraPosition);
        camera.quaternion.copy(followModeLastCameraQuaternion);
    }
}
