...
function addObstacles(scene, amount, area) {
    ...
    for (let i = 0; i < amount; i++) {
        ...
        const size = 5 + Math.random() * 20; // Random size between 2 and 10
        const cubeGeometry = new THREE.BoxGeometry(size, 60, size);

        // Random shade of gray for the obstacle
        const grayShade = Math.random() * 0.5 + 0.2; // Range from 0.2 (dark gray) to 0.7 (lighter gray)
