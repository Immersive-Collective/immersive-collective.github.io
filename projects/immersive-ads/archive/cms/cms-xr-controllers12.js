...
const brakeDeceleration = 0.025;  // Strong braking effect
const maxEngineGain = 0.5;        // Max sound volume
const minEngineGain = 0.05;       // Minimum engine hum
...
function adjustEngineSound() {
    const speedInKmH = Math.abs(vehicleVelocity.z) * 3.6;
    const speedRatio = Math.min(speedInKmH / maxSpeed, 1);  // Normalize to range [0, 1]

    const lowFreq = 20 + speedRatio * 200;  // Engine low hum frequency
    const midFreq = 100 + speedRatio * 400;  // Mid growl frequency
    const gainValue = minEngineGain + speedRatio * (maxEngineGain - minEngineGain);  // Volume adjustment
    const cutoffFreq = 800 + speedRatio * 1200;  // Open up high frequencies as speed increases
    ...
    lowPassFilter.frequency.setValueAtTime(cutoffFreq, audioContext.currentTime);  // Adjust filter cutoff
...
function createNoiseBuffer() {
    ...
    for (let i = 0; i < bufferSize; i++) {
        data[i] = (Math.random() * 2 - 1) * 0.05;  // Subtle noise (hiss)
    }
...
//     // Idle sound frequencies
//     engineLowOscillator.type = "sawtooth";      // Low rumble
//     engineMidOscillator.type = "triangle";      // Mid growl
//     engineLowOscillator.frequency.setValueAtTime(20, audioContext.currentTime);  // Idle low freq
//     engineMidOscillator.frequency.setValueAtTime(100, audioContext.currentTime);  // Idle mid freq

//     gainNode.gain.setValueAtTime(minEngineGain, audioContext.currentTime);  // Low volume hum at idle
//     lowPassFilter.type = "lowpass";
//     lowPassFilter.frequency.setValueAtTime(1000, audioContext.currentTime);   // Cutoff for initial hum
...
function adjustEngineSound() {
    const speedInKmH = Math.abs(vehicleVelocity.z) * 3.6;  // Convert speed to km/h
    const speedRatio = Math.min(speedInKmH / maxSpeed, 1);  // Normalize the speed to a ratio between 0 and 1

    const lowFreq = 20 + speedRatio * 200;  // Low rumble increases with speed
    const midFreq = 100 + speedRatio * 400;  // Mid growl increases with speed
    const gainValue = minEngineGain + speedRatio * (maxEngineGain - minEngineGain);  // Volume adjustment based on speed
...
function handleCollision() {
    if (!collisionDetected) {
        console.log("Collision detected: resetting engine sound.");
        vehicleVelocity.set(0, 0, 0);  // Stop the vehicle
        adjustEngineSound();  // Reset engine sound to idle
        playCollisionSound();  // Play collision sound
        collisionDetected = true;

        setTimeout(() => {
            collisionDetected = false;  // Allow new collisions after a delay
        }, 1000);
    }
}
...
// Update engine power based on inputs
// Main engine sound update function
// function updateEngineSound(deltaTime) {
//     const speedInMetersPerSecond = Math.abs(vehicleVelocity.z);  // Get current vehicle speed (absolute value)
//     const speedInKmH = speedInMetersPerSecond * 3.6;  // Convert to km/h
//     const speedRatio = Math.min(speedInKmH / maxSpeed, 1);  // Normalize to a range of [0, 1]

//     const lowFreq = 20 + speedRatio * 200;  // Engine low frequency hum
//     const midFreq = 100 + speedRatio * 400;  // Mid-range growl frequency
//     const gainValue = minEngineGain + speedRatio * (maxEngineGain - minEngineGain);  // Volume adjustment based on speed

//     // Apply the frequency and gain updates
//     engineLowOscillator.frequency.setValueAtTime(lowFreq, audioContext.currentTime);
//     engineMidOscillator.frequency.setValueAtTime(midFreq, audioContext.currentTime);
//     gainNode.gain.setValueAtTime(gainValue, audioContext.currentTime);
// }
...
// Play collision sound and reset engine sound
function playCollisionSound() {
    ...
    collisionOscillator.type = "square";  // Sharp sound
    collisionOscillator.frequency.setValueAtTime(100, audioContext.currentTime);  // Clank sound frequency
    collisionOscillator.frequency.exponentialRampToValueAtTime(20, audioContext.currentTime + 0.5);  // Drop-off

    collisionGain.gain.setValueAtTime(0.5, audioContext.currentTime);  // Start loud
    collisionGain.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + 1);  // Fade out
    ...
    collisionOscillator.stop(audioContext.currentTime + 1);  // Play for 0.3 seconds
}
    ...
    obstacleGroup?.children.forEach((obstacle) => {
        ...
        if (carBoundingBox.intersectsBox(obstacleBoundingBox)) {
            ...
            if (!collisionDetected) {
                ...
                playCollisionSound();  // Trigger sound on collision
            }
            hasCollided = true;
        }
    });
