...
//     const obstacleGroup = scene.getObjectByName("ObstacleGroup"); // Get the obstacle group
...
//             damagePoints += 10; // Add damage points continuously
...
//             updateDamageUI(); // Update damage points and collisions display
//         }
//     });
...
function createSmokeTexture() {
    ...
    const numCircles = 10 + Math.random() * 10;
...
// Function to add a smoke emitter
function addSmokeEmitter(collisionPoint) {
    const smokeSprite = createSmokeSprite(collisionPoint);
    scene.add(smokeSprite);

    const startTime = performance.now();
    const duration = 2000; // 2 seconds duration

    activeSmokeEmitters.push({
        sprite: smokeSprite,
        startTime,
        duration,
    });
...
// Function to create smoke sprite
function createSmokeSprite(position) {
    const sprite = new THREE.Sprite(smokeMaterial.clone()); // Clone material to avoid sharing issues

    // Offset initial position to be slightly above the car
    sprite.position.copy(position).add(new THREE.Vector3(0, 1, 0)); // Raise 1 unit above the car (adjust as needed)
    
    sprite.scale.set(2.5, 2.5, 2.5); // Control smoke size
    sprite.material.opacity = 1; // Initial opacity
    return sprite;
}

// Function to update all active smoke emitters (adding upward movement)
function updateSmokeEmitters(deltaTime) {
    const currentTime = performance.now();
    for (let i = activeSmokeEmitters.length - 1; i >= 0; i--) {
        const emitter = activeSmokeEmitters[i];
        const elapsedTime = currentTime - emitter.startTime;
        const t = elapsedTime / emitter.duration;

        if (t >= 1) {
            // Remove the sprite when duration is over
            scene.remove(emitter.sprite);
            emitter.sprite.material.dispose();
            emitter.sprite.geometry.dispose();
            activeSmokeEmitters.splice(i, 1); // Remove from array
        } else {
            // Gradually fade out the smoke
            emitter.sprite.material.opacity = 0.7 * (1 - t);

            // Move the sprite upward over time
            const upwardSpeed = 0.5; // Controls how fast the smoke rises
            emitter.sprite.position.y += upwardSpeed * deltaTime; // Increment y-position

            // Gradually increase the scale to prevent visual cropping
            const scaleMultiplier = 1 + t * 0.5; // Increase size as smoke rises
            emitter.sprite.scale.set(1.5 * scaleMultiplier, 1.5 * scaleMultiplier, 1.5 * scaleMultiplier);
        }
    }
}
...
//                 // Optional: Flash the car red for 1 second to indicate a crash
//                 vehicleContainer.children.forEach((mesh) => {
//                     if (mesh.material) mesh.material.color.set(0xff0000);
//                 });
...
//             // Add color flash feedback to the entire vehicle hierarchy
//             vehicleContainer.traverse((node) => {
//                 if (node.isMesh && node.material) {
//                     node.material.color.set(0xff0000); // Flash red
//                 }
//             });
...
//                 vehicleContainer.traverse((node) => {
//                     if (node.isMesh && node.material) {
//                         node.material.color.set(0xffffff); // Reset color to white
//                     }
//                 });
//             }, 500); // Flash duration
...
// Redesigned function to combine orbit control and follow mode
function updateCameraFollow(deltaTime) {
    if (isCameraFollowingCar && !isCameraMoving && !isFlying) {
        ...
        // Update OrbitControls center to always track the car position
        controls.target.copy(carWorldPos);

        // Keep the same relative position as when follow mode started
        ...
        relativeOffset.applyQuaternion(carRotation); // Rotate offset based on car's rotation

        const targetPosition = carWorldPos.clone().add(relativeOffset);

        // Smooth interpolation for the camera position
        const lerpFactor = Math.min(0.1, deltaTime * 2); // Faster interpolation for responsiveness
        camera.position.lerp(targetPosition, lerpFactor);

        // Maintain focus on the car
        camera.lookAt(carWorldPos);

        controls.update(); // Ensure the controls stay updated
    } else {
        // === FOLLOW MODE OFF ===
        controls.target.copy(controls.target); // Keep the last car position as the orbit center
