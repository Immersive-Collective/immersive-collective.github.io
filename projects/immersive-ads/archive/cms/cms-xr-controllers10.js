...
// Function to apply visual damage effect (displacement)
function applyMeshDisplacement(mesh, intensity) {
    ...
    for (let i = 0; i < vertexCount; i++) {
        // Generate random displacement vectors in all directions
        const randomDirection = new THREE.Vector3(
            (Math.random() - 0.015) * intensity, // X direction
            (Math.random() - 0.015) * intensity * 0.05, // Y direction (slightly less for "flattening" effect)
            (Math.random() - 0.015) * intensity // Z direction
        );

        const x = positionAttribute.getX(i) + randomDirection.x * 0.002; // Less intense by multiplying by 0.02
        const y = positionAttribute.getY(i) + randomDirection.y * 0.002; // Y has a smaller multiplier
