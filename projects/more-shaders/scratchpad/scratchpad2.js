...
function initGL() {
    ...
    const fragmentShaderSource = `
        ...
        uniform float u_brushAspect; // Correct aspect ratio for the brush

        void main() {
            vec4 baseColor = texture2D(u_texture, v_texCoord);
            
            // Corrected aspect ratio adjustment
            vec2 overlayUV = (v_texCoord - u_overlayPos) / vec2(u_brushSize, u_brushSize / u_brushAspect) + vec2(0.5);
...
function createTexture(image, isOverlay = false) {
    ...
    if (isOverlay) {
        overlayTexture = tex;
        brushAspect = image.width / image.height; // Store aspect ratio
    } else {
        texture = tex;
    }
...
function drawScene() {
    ...
    gl.uniform1f(brushAspectLocation, brushAspect); // Pass correct aspect ratio
