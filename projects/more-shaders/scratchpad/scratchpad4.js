...
const brushFiles = ["images/brushes/1.png", "images/brushes/2.png", "images/brushes/3.png"];
let brushTextures = [];
let brushAspects = [];
...
let brushSize = 0.2; // normalized to canvas width
...
canvas.addEventListener("mousemove", (event) => {
  const pos = getMousePos(event);
  overlayPosition = [pos.x / canvas.width, pos.y / canvas.height];
  if (isDrawing) { drawBrushStrokeToPaintLayer(pos.x, pos.y); }
});
...
function drawBrushStrokeToPaintLayer(x, y) {
  ...
  const x1 = x - halfW, y1 = y - halfH;
  const x2 = x + halfW, y2 = y + halfH;
...
function drawBrushOverlay() {
  ...
  // Pass the tint color to the overlay shader.
  ...
  const halfW = brushW / 2;
  const halfH = brushH / 2;
  const x1 = posX - halfW, y1 = posY - halfH;
  const x2 = posX + halfW, y2 = posY + halfH;
